...
spark = SparkSession.builder.appName("Parquet2CSVActivity").config("spark.executor.memory", "2g") .config("spark.executor.cores", "2").config("spark.driver.memory", "2g").getOrCreate()
...
# par_name = f"{str(uuid.uuid4())}.csv"
# par_path = os.path.join(config["output_path"]['filePath'], par_name)
# abs_par_path = os.path.abspath(par_path)
abs_csv_path = os.path.abspath(os.path.join(config["output_path"]['filePath'], str(uuid.uuid4())))


#  Save as CSV
df.coalesce(1).write.option("header", "true").csv(abs_csv_path)
